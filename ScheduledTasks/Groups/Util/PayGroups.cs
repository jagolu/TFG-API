    ...
    public static class PayGroups
    {
        public static void pay(ApplicationDBContext _context)
        {
            ...
            ).ToList().ForEach(group =>
            {
                ...
                Areas.Home.Util.GroupNew.launch(null, group, Areas.Home.Models.TypeGroupNew.PAID_PLAYERS_GROUPS, false, _context);

                group.users.ToList().ForEach(u =>
                {
                    u.coins += moreCoins;
                    _context.Entry(u).Reference("User").Load();
                    Areas.Home.Util.GroupNew.launch(u.User, group, Areas.Home.Models.TypeGroupNew.PAID_PLAYERS_USER, false, _context);
                });
            });
